...
[[ -z $HOST_CONFIG_DIR ]] && HOST_CONFIG_DIR="/tank/anbaar/projects/hobbies/zmk_firmwares/zmk-config-jaan"
...
if [[ $RUNWITH_DOCKER = true ]]; then
    ...
    if [[ $CLEAR_CACHE = true ]]; then
        ...
        sudo rm -rf "$local_zmk/.west"
        sudo rm -rf "$local_output"/*
        sudo /usr/bin/rm -rf /tank/anbaar/projects/hobbies/zmk_firmwares/zmk-config-jaan/output/*
    fi
